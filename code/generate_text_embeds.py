...
def _caption_path(text_root: Path, image_name: str) -> Path:
    wnid = image_name.split("_")[0]
    return text_root / wnid / f"{image_name}_caption.txt"


@torch.no_grad()
def main():
    p = argparse.ArgumentParser(description="Generate CLIP text embeddings (per-sample .pt) for DreamDiffusion.")
    ...
    p.add_argument("--text_root", type=str, default="datasets/text_data")
    p.add_argument("--output_root", type=str, default="datasets/embeddings/text")
    ...
    def flush_batch():
        nonlocal batch_image_names, batch_dataset_indices, batch_texts, manifest
        if not batch_texts:
            return
        tokens = tokenizer(
            batch_texts,
        ...
        for image_name, ds_idx, emb, text in zip(batch_image_names, batch_dataset_indices, embeds, batch_texts):
            wnid = image_name.split("_")[0]
    ...
    for ds_idx in tqdm(split_indices, desc=f"Processing {args.split_name} text embeddings"):
        ...
        cap_path = _caption_path(text_root, image_name)
            ...
            missing.append(
                {
                    ...
                    "wnid": image_name.split("_")[0],
                    "dataset_index": ds_idx,
                    "missing_path": str(cap_path.as_posix()),
                }
            )
            continue
    ...
    print(f"[text] split={args.split_name} subject={args.subject} exported={len(manifest)} missing={len(missing)}")
